<template>
    <div class="wrapper">
        <div class="avatar">
            ...
            <p>
                Alyona
                <span v-if="isTyping" class="isTyping"
                    ><p>is typing...</p></span
                >
            </p>
        </div>
        <div class="chat-wrapper">
            <section class="chat">
                ...
                <div v-for="day in chat" :key="day.index">
...
export default {
    ...
    methods: {
        handleScroll() {
            ...
            if (el.scrollTop + el.clientHeight >= el.scrollHeight) {
                ...
                for (let day in this.chat) {
                    let res = this.chat[day].filter(
                        (msg) => msg.to != 1 && msg.from
                    );
                    res.map((msg) =>
                        setTimeout(() => {
                            msg.is_notified = 1;
            ...
            Echo.private(`chat.${this.$store.state.currentUserId}`).listen(
                ...
                (data) => {
                    axios.get(`/api/chat`).then((res) => {
                        if (
                            data.message.from != this.$store.state.currentUserId
                        ) {
                            this.init(res);
                            this.isTyping = false;
                            const el = document.querySelector(".chat");
        ...
        disconnect() {
            Echo.private(
                `chat.${this.$store.state.currentUserId}`
        ...
        pusherWhisper() {
            Echo.private(
                ...
                if (
                    1 == e.idFrom &&
                    e.idTo === this.$store.state.currentUserId
                ) {
                    this.isTyping = true;
